...
│   ├── setup.ps1              # Dependency check installer (Windows)
│   └── setup.sh               # Dependency check installer (macOS/Linux)
├── interface/                 # Visual dashboard console
│   ├── index.html             # Glassmorphic layout client
│   ├── theme.css              # Premium dark-mode stylesheet
│   ├── app.js                 # Polling bindings & reactive controls
│   └── dashboard_layout.json  # UI layout metadata parameters
...
*   **One-Click Installers (`setup.sh`/`setup.ps1`):** Scripts verifying Node, PM2, and local Ollama, setting standard user environment bindings and configuring the local folder layout.
...
### 3. The Documentation Manuals (`/docs`)
...
2. **Execute installer** to verify system dependencies:
   ```powershell
